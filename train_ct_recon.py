...
model_name = os.path.join(output_folder, output_subfolder + '/slice{}_img{}_proj{}_{}_{}_{}_{}_{}_lr{:.2g}_encoder_{}' \
    .format(config['img_slice'], config['img_size'], config['num_proj'], config['model'], \
...
# Load pretrain model
if opts.pretrain:
    model_path = config['pretrain_model_path'].format(config['img_slice'], config['img_size'], \
...
for it, (grid, image) in enumerate(data_loader):
    ...
    for iterations in range(max_iter):
        ...
        train_projs = ct_parallel_project_2d_batch(train_output, thetas)
